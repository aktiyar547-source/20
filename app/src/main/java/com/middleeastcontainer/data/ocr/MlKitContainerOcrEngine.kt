...
class MlKitContainerOcrEngine @Inject constructor(
    ...
    private fun collectFrom(
        ...
        // 3. Columns, gathered across the WHOLE image rather than per block.
        //
        // A number painted down a container door arrives as one short line per
        // character, and the recogniser routinely splits those into separate
        // blocks — the owner code, the serial and the boxed check digit each
        // becoming their own. Grouping within a block therefore never joins
        // them, which is why an upright vertical number read as nothing.
        val allLines = text.textBlocks.flatMap { it.lines }
        // A stacked number sometimes arrives as one line per character and
        // sometimes with two or three grouped together, depending on spacing.
        // Both shapes are tried, because which one appears is not predictable.
        for (column in verticalRuns(allLines)) {
     ...
     */
    /**
     * Groups short lines that sit above one another into columns.
     *
     * Fed every line in the image rather than one block's worth: a number
     * painted down a door is habitually split by the recogniser into separate
     * blocks — owner code, serial, and boxed check digit — and grouping within a
     * block therefore joins nothing. Any accidental column formed from unrelated
     * text is discarded by the check digit.
     */
    private fun verticalRuns(lines: List<Text.Line>): List<List<Text.Line>> {
        val boxed = lines
            .filter { it.boundingBox != null && it.text.trim().length in 1..SHORT_LINE }
        ...
        for (line in boxed) {
            ...
            // Match against ANY open column, not merely the most recent: two
            // numbers side by side on the same door interleave when sorted by
            // height, and only-the-last would braid them together.
            val run = runs.firstOrNull { current ->
                val last = current.last().boundingBox!!
                val tolerance = maxOf(last.width(), box.width()) * COLUMN_TOLERANCE
                kotlin.math.abs(last.centerX() - box.centerX()) <= tolerance &&
                    box.top >= last.top &&
    ...
    private companion object {
        ...
        /** A stacked character arrives as a very short line, often just one. */
        const val SHORT_LINE = 4

        /** Fewer than this is not a stacked number. */
        const val MIN_COLUMN = 4

        /** Horizontal drift allowed within a column, as a fraction of width. */
        const val COLUMN_TOLERANCE = 1.2

        /**
         * Vertical gap allowed between characters, in element heights.
         *
         * Generous on purpose: a painted number leaves visible space between the
         * owner code, the serial and the boxed check digit, and a tight limit
         * splits one number into three fragments that each fail validation.
         */
        const val COLUMN_GAP = 4.0
