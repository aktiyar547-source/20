...
class ContainerRepositoryImpl @Inject constructor(
    ...
    override fun search(query: String): Flow<List<Container>> {
        val trimmed = query.trim()
        return if (trimmed.isEmpty()) observeAll()
        else containerDao.search(trimmed).map { list -> list.map { it.toDomain() } }
    }
