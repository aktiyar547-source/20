...
fun SweepScreen(
    ...
    Box(Modifier.fillMaxSize().background(Color.Black)) {
        ...
        SweepCamera(
            ...
            // Reading happens behind the viewfinder, so the shutter waits only
            // for a decision the inspector actually has to make.
        ...
        Column(
            ...
            // A tick of scale when the count rises: confirmation felt at a
        ...
        // A quiet badge, not a blocking spinner: the inspector can keep shooting.
        ...
        Row(
            ...
            Button(
                onClick = {
                    // Gaps are allowed, but never closed by accident.
                    if (unread.isEmpty()) viewModel.finish(onFinished) else confirmFinish = true
                },
                // Closing while frames are still being read would discard them.
                enabled = state.processing == 0,
            ) { Text(if (state.processing > 0) "Reading…" else "Finish") }
        }
    }
