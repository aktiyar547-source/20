...
fun CameraScreen(
    ...
    fun shoot() {
        if (capturing) return
        capturing = true
        // Felt confirmation: in bright sun, and while moving round a container,
        // the screen is often not being watched.
    ...
    Box(Modifier.fillMaxSize().background(Color.Black)) {
        ...
        Box(
            Modifier
                ...
                .padding(16.dp)
                .size(48.dp)
                .background(if (torchOn) BrandGold else Color(0x66000000), CircleShape)
                .clickable {
                    torchOn = !torchOn
            ...
            Text("☀", color = if (torchOn) Color.Black else Color.White, fontSize = 22.sp)
        }

        // Last stored photo: proof the shot landed, without interrupting shooting.
        AnimatedVisibility(
            visible = viewModel.isBurst && state.lastThumbnail != null,
            enter = fadeIn(tween(Motion.MEDIUM, easing = Motion.Enter)),
