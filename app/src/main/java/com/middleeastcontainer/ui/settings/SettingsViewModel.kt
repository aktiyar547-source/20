...
data class SettingsUiState(
    ...
    /** The folders exist on the phone and are writable. */
    val photoFolderReady: Boolean = false,
...
class SettingsViewModel @Inject constructor(
    ...
    init {
        viewModelScope.launch {
            ...
            _state.value = SettingsUiState(
                ...
                photoFolderReady = fileStore.ensureFolders(),
    ...
    fun refreshPhotoFolderAccess() {
        // Create the folders the moment access exists, so the inspector sees
        // /OCR2 on the phone immediately rather than after the first photo.
        val created = fileStore.ensureFolders()
        _state.value = _state.value.copy(
            hasPhotoFolderAccess = fileStore.hasRootAccess,
            photoFolderReady = created,
            photoFolderPath = fileStore.sharedRootPath(),
        )
    }
