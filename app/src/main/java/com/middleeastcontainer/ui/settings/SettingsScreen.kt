        ...
        Column(
            ...
            Card(Modifier.fillMaxWidth()) {
                Column(Modifier.padding(14.dp)) {
                    if (state.hasPhotoFolderAccess && state.photoFolderReady) {
                        // Names the real path and confirms the folders exist, so
                        // the inspector can verify it over USB rather than trust it.
                        Text("Saving to ${state.photoFolderPath}", color = VerifiedGreen)
                        Text(
                            "Inspection and Inventory folders are ready inside it. " +
                                "Connect by USB and look for OCR2 at the top level.",
                            style = MaterialTheme.typography.bodySmall,
                            color = MaterialTheme.colorScheme.onSurfaceVariant,
                        )
                    } else if (state.hasPhotoFolderAccess) {
                        // Permission held but the folders would not create: rare,
                        // and worth saying plainly rather than failing at capture.
                        Text(
                            "Access granted, but ${state.photoFolderPath} is not writable. " +
                                "Some phones (Xiaomi/MIUI especially) block the top of storage " +
                                "separately — check Security \u2192 Permissions \u2192 MECRC. " +
                                "Photos are still saved safely inside the app until this is fixed.",
                            color = MaterialTheme.colorScheme.error,
                        )
                        Text(
                            "Photos will save inside the app's private folder instead. " +
                                "Check the phone has free space.",
                            style = MaterialTheme.typography.bodySmall,
                            color = MaterialTheme.colorScheme.onSurfaceVariant,
                        )
                    } else {
