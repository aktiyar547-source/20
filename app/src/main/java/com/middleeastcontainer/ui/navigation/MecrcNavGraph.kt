...
fun MecrcNavGraph(navController: NavHostController) {
    ...
    NavHost(
        ...
        // Going deeper slides in from the right, coming back reverses it — so the
        // hierarchy is felt rather than read. A quarter of the width, not the
        // whole screen: enough to show direction without the wait.
        enterTransition = {
            slideInHorizontally(
                initialOffsetX = { it / 4 },
                animationSpec = tween(Motion.SCREEN, easing = Motion.Enter),
            ) + fadeIn(tween(Motion.SCREEN, easing = Motion.Enter))
        },
        exitTransition = {
            slideOutHorizontally(
                targetOffsetX = { -it / 6 },
                animationSpec = tween(Motion.SCREEN, easing = Motion.Exit),
