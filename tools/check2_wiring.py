...
for p, f in files.items():
    is_module = "@Module" in f["src"]
    # `abstract fun x(): T` is a Hilt binding only inside a @Module. The same
    # shape inside a RoomDatabase is a DAO accessor, and treating it as a
    # provider made this check vacuous for every DAO: removing a @Provides
    # still passed.
    if is_module:
        providable |= set(re.findall(r'abstract fun \w+\([^)]*\):\s*(\w+)', f["code"]))
    providable |= set(re.findall(r'@Provides[\s\S]{0,120}?fun \w+\([^)]*\)\s*:\s*(\w+)', f["src"]))
    # `@Provides fun inventoryDao(db: MecrcDatabase) = db.inventoryDao()` — the
    # provided type is the accessor's return type, so read it from the database
    # class rather than from the lowercase method name.
    for m in re.finditer(r'@Provides[\s\S]{0,120}?fun \w+\([^)]*\)\s*=\s*\w+\.(\w+)\(\)', f["src"]):
        accessor = m.group(1)
        for other in files.values():
            rm = re.search(rf'abstract fun {accessor}\(\):\s*(\w+)', other["src"])
            if rm:
                providable.add(rm.group(1))
...
providable |= {"MecrcDatabase", "MecrcApi", "OkHttpClient"}
